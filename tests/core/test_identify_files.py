...
class TestSFId:
    ...
    def test_subprocess_error(self, docx_info):
        with pytest.raises(IdentificationError), patch(
            "subprocess.run",
            side_effect=CalledProcessError(returncode=1, cmd="Fail", stderr=b"Fail"),
        ):
            sf_id(docx_info)
...
class TestCustomId:
    def test_lwp(self, temp_dir):
        ...
        lwp_file.write_bytes(
            bytes.fromhex("576F726450726F0DFB000000000000" "000005985C8172030040CCC1BFFFBDF970"),
        )
        lwp_id = Identification(puid=None, signature=None, warning="this is a warning")
    ...
    def test_123(self, temp_dir):
        ...
        _123_id = Identification(puid=None, signature=None, warning="this is a warning")
    ...
    def test_word_markup(self, temp_dir):
        ...
        word_markup.write_bytes(
            bytes.fromhex("6e2070726f6769643d22576f72642e446f63756d656e74223f3e3c773a776f726444"),
        )
        ...
        assert new_id.warning is None
        word_markup_wrong_suffix = word_markup.rename(word_markup.with_suffix(".fail"))
        new_id_wrong_suffix = custom_id(word_markup_wrong_suffix, word_markup_id)
    ...
    def test_excel_markup(self, temp_dir):
        ...
        assert new_id.warning is None
        excel_markup_wrong_suffix = excel_markup.rename(excel_markup.with_suffix(".test"))
        new_id_wrong_suffix = custom_id(excel_markup_wrong_suffix, excel_markup_id)
        assert new_id_wrong_suffix.puid == new_id.puid
        assert new_id_wrong_suffix.signature == new_id.signature
        assert new_id_wrong_suffix.warning == "Extension mismatch"
    ...
    def test_mmap(self, temp_dir):
        ...
        assert new_id.warning is None
        mmap_markup_wrong_suffix = mmap_markup.rename(mmap_markup.with_suffix(".test"))
        new_id_wrong_suffix = custom_id(mmap_markup_wrong_suffix, mmap_markup_id)
        assert new_id_wrong_suffix.puid == new_id.puid
        assert new_id_wrong_suffix.signature == new_id.signature
        assert new_id_wrong_suffix.warning == "Extension mismatch"
    ...
    def test_gif(self, temp_dir):
        ...
        assert new_id_dict[gif_file].signature == "GIF 1989a"
        ...
        assert fail_id_dict[fail_gif_file].signature == "GIF 1989a"
        assert fail_id_dict[fail_gif_file].warning == "Extension mismatch"

    def test_nsf(self, temp_dir):
        nsf_file = temp_dir / "mock.nsf"
        nsf_file.write_bytes(bytes.fromhex("1a000004000029000000bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"))
...
class TestIsPreservable:
    ...
    def test_is_preservable_binary_file_true(self, python_wiki_binary_file: ArchiveFile, get_log):
        assert is_preservable(python_wiki_binary_file, get_log)[0] is True

    def test_is_preservable_binary_file_false(self, very_small_binary_file: ArchiveFile, get_log):
    ...
    def test_is_preservable_non_binary_file(self, non_binary_file: ArchiveFile, get_log):
        assert is_preservable(non_binary_file, get_log)[0] is True

    def test_image_is_preservable_on_pillow_exception(self, very_small_binary_file, lock, get_log):
        """DocTest."""
        # If pillow cannot parse an image file, it should
        # still be marked as preservable
        # but looked at manually at some point.
        assert image_is_preservable(very_small_binary_file, lock, get_log) is True
    ...
    def test_decrompresionbomb_log_error(self, binary_file, monkeypatch, lock, get_log):
        ...
        monkeypatch.setattr("digiarch.core.identify_files.check_if_preservable", mock_open)
        ...
        with open(pathToFile, encoding="utf-8") as file:
            for line in file.readlines():
                if "ERROR: The file image_file.png threw " "a decompresionbomb error\n" in line:
                    assert True
                    return
                else:
                    continue
    ...
    def test_decrompresionbomb_log_warning(self, binary_file, monkeypatch, lock, get_log):
        ...
        with open(pathToFile, encoding="utf-8") as file:
            for line in file.readlines():
                if "WARNING: The file image_file.png threw a " "decompresionbomb warning\n" in line:
                    assert True
                    return
                else:
                    continue
